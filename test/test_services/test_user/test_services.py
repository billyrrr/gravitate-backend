...
class UserEndPointTest(TestCase):
    ...
    def setUp(self):
        ...
        auth.create_user(app=context.Context.firebase_app, uid = test_user1_dict["uid"])
        path = '/users/' + test_user1_dict["uid"]
        r = self.app.post(path=path, json=json.dumps(test_user1_dict))
        assert r.status_code == 200

    def tearDown(self):
        auth.delete_user(app=context.Context.firebase_app, uid=test_user1_dict["uid"])
...
class UserCreationEndpointTest(TestCase):
    ...
    def setUp(self):
        main.app.testing = True
        self.app = main.app.test_client()
        auth.create_user(app=context.Context.firebase_app, uid=test_user2_dict["uid"])

    def tearDown(self):
        auth.delete_user(app=context.Context.firebase_app, uid=test_user2_dict["uid"])
...
class FirebaseUserTest(TestCase):
    ...
    def testGetFirebaseInfo(self):
        user = auth.get_user(userDict["uid"], app=context.Context.firebase_app)
    ...
    def testUpdateUser(self):
        auth.update_user(userDict["uid"],
                         ...
                         disabled=False,
                         app=context.Context.firebase_app
                         )
        user = UserDao().get_user_by_id(userDict["uid"])
